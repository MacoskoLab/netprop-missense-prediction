...
# New functions for weight transformation
def threshold_grn_weights(
    W: pd.Series, S: pd.Series, threshold: float = 0.6
) -> pd.Series:
    # align indices
...
def sigmoid_grn_weights(
    W: pd.Series, S: pd.Series, k: float = k, t: float = t
) -> pd.Series:
    # align indices
    W_aligned, S_aligned = W.align(S, join="inner")
...
for cell_id, group in am_df.groupby("sample"):
    ...
    initial_impacts = {g: logistic_transform(am_scores_cell[g], k, t) for g in genes}
    # Propagate impacts
